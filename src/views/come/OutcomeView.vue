      ...
      <table class="table">
        ...
        <tbody>
          <tr v-for="outcome in pageData" @click="openDetailModal(outcome.id)">
            ...
            <td>{{outcome.title}}</td>
            <td>{{outcome.money}} 원</td>
            <td>{{outcome.method}}</td>
          </tr>
        </tbody>
      </table>
    </div>
...
export default {
    ...
    setup() {
        ...
        return {onChangeDate, onChangeMonth, onChangeStartDate, onChangeEndDate, prevMonth, nextMonth, date, month, startDate, endDate, dateType, pageState, totalPageCount, pageData, categoriesStore, comesStore, onChangePage, onPrevPage, onNextPage, categorySelect, onChangeDateType, onChangeCategory, openModal, openDetailModal, closeModal, openModal1, openCreateModal, closeCreateModal};
    }
}
</script>
<style scoped>
